...
* Setflask config variables from the environment variables we loaded (secret key, uploads folder, maximum content length if we are doing file uploading, session cookie settings (http only)
